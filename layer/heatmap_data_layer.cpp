...
HeatmapDataLayer<Ftype, Btype>::HeatmapDataLayer(const LayerParameter& param,
    size_t solver_rank)
  : DataLayer<Ftype, Btype>(param, solver_rank) {}

template <typename Ftype, typename Btype>
void HeatmapDataLayer<Ftype, Btype>::DataLayerSetUp(
  ...
  const HeatmapDataParameter& heatmap_data_param = param.heatmap_data_param();
  ...
  label_map_file_ = heatmap_data_param.label_map_file();
  heatmap_c_ = heatmap_data_param.heatmap_c();
  heatmap_h_ = heatmap_data_param.heatmap_h();
  heatmap_w_ = heatmap_data_param.heatmap_w();
  visualise_ = heatmap_data_param.heatmap_visual();
  sigma_ = heatmap_data_param.heatmap_sigma();
  ...
  if (this->output_labels_) {
    ...
    if (has_anno_type_) {
      ...
      if (anno_data_param.has_anno_type()) {
        // If anno_type is provided in HeatmapDataParameter, replace
  ...
  LOG(INFO) << this->print_current_device() << " Output data size: "
      ...
      << top[0]->height() << ", "
      << top[0]->width();
  LOG(INFO) << this->print_current_device() << " Output label size: "
      << top[1]->num() << ", "
      << top[1]->channels() << ", "
      << top[1]->height() << ", "
      << top[1]->width();
...
template <typename Ftype, typename Btype>
void HeatmapDataLayer<Ftype, Btype>::load_batch(Batch* batch, int thread_id, size_t queue_id) {
  ...
  const HeatmapDataParameter& heatmap_data_param =
      this->layer_param_.heatmap_data_param();
  const TransformationParameter& transform_param =
    this->layer_param_.transform_param();
				...
				for (int a = 0; a < anno_group.annotation_size(); ++a) {
					...
					for (int i = 0; i < heatmap_h_; i++) {
						for (int j = 0; j < heatmap_w_; j++) {
							...
							Ftype gaussian = 0.5 * ( pow(i - py_, 2.0) + pow(j - px_, 2.0) ) * pow(1 / sigma, 2.0);
							if (gaussian > 4.6052) 
								continue;
...
INSTANTIATE_CLASS_FB(HeatmapDataLayer);
REGISTER_LAYER_CLASS_R(HeatmapData);
